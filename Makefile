...
migrate-create:
	...
	migrate create -ext sql -dir $(PWD)/$(APPPATH)/account/migrations -seq -digits 5 $(NAME)

migrate-up:
	migrate -source file://$(PWD)/$(APPPATH)/account/migrations -database postgres://postgres:password@localhost:$(PORT)/postgres?sslmode=disable up $(N)

migrate-down:
	migrate -source file://$(PWD)/$(APPPATH)/account/migrations -database postgres://postgres:password@localhost:$(PORT)/postgres?sslmode=disable down $(N)
